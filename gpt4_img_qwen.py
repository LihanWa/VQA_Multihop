...
#         ]
#     }]
#     response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
#     print(response)
def create_payload_edge(image: str, prompt: str, label_sentence: str , max_tokens=200, detail="high"):
        ...
        
        {
            ...
            "content": [
                
                {
                    ...
                    "text": "My question is: "+prompt+" Answer a complete sentence instead of just 'yes' or 'no'."
                },
                {
                    'image': local_file_path
                },
...
def create_payload_final_ans(image: str, prompt: str, label_sentence: str, context: str , max_tokens=200, detail="high", temperature=0.0):
    """Creates the payload for the API request."""
    # print(context)
...
- Responses should be concise. For questions about "What" or "Where," the answer should be no more than two words, indicating either the object or the place.
...
def create_payload_size_tool(image: str, question: str, label_sentence: str , max_tokens=200, detail="high", temperature=0.0):
    """Creates the payload for the API request."""
...
In the question, brackets '[]' are used to mark different instances of an object.'''+label_sentence+'''The question is as follows:'''
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def create_payload_identity_tool(image: str, question: str, label_sentence: str , max_tokens=200, detail="high", temperature=0.0):
    """Creates the payload for the API request."""
...
def mmbench_payload(image: str, question: str, options: str , max_tokens=200, detail="high", temperature=0.0):
    """Creates the payload for the API request."""
    # print(context)
    local_file_path = f'file://{image}'
...
def judge_few_objs(image: str, prompt: str, entity_str , max_tokens=200, detail="high", temperature=0.0):
    """Creates the payload for the API request."""
    # print(context)
    # spe_words = ['photo', 'image', 'photograph', 'Photo', 'Image', 'Photograph', 'place']
    # prompt_words = prompt.split(' ')
...
def create_payload_node(image: str, prompt: str, label_sentence:str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
    spe_words = ['photo', 'image', 'photograph', 'Photo', 'Image', 'Photograph', 'place']
    prompt_words = prompt.split(' ')
    for i, prompt_word in enumerate(prompt_words):
            ...
            "content": [
                {
                    ...
                    "text": "My question is: "+prompt+" Answer a full sentence."
                },
                {
                    'image': local_file_path
                },
...
def create_payload_normal(image: str, prompt: str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def find_other_obj(image: str, prompt: str,entity:str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
...
def find_not_found_obj(image: str, prompt: str,entity:list , max_tokens=200, detail="high"):
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def conclude_answer(image: str, question: str,answer:str , max_tokens=200, detail="high"):
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def judge_obj(image: str, prompt: str,entity:list , max_tokens=200, detail="high"):
...
def check_mark(image: str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
...
def vcr_ans_q(image: str, question: str,options:str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
...
AI assistant should choose from "A","B",C","D". For instance AI assistant should say "A". '''
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def vcr_des(image: str,question: str,objects:str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
...
The format of AI assistant should be like: <Description of ..: .. Description of ..: ..> '''
    ...
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
def vcr_des_ans(image: str,question: str,multi_choice:str,objects:str , max_tokens=200, detail="high"):
    """Creates the payload for the API request."""
...
def create_payload_qwen_zeroshot(image: str, question: str ):
    """Creates the payload for the API request."""
            ...
            "content": [
                {
                    ...
                    "text": '''AI assistant is tasked with answering a VQA question based on an image. /
The answer of AI assistant should be as short as possible to match the format of correct answer, which means usually one word or two words are enough for the answer.'''
                }
            ]
        },
        {
...
def create_payload_find_important_obj(image: str, question: str , objs:str):
    """Creates the payload for the API request."""
            ...
            "content": [
                {
                    ...
                    "text": '''AI assistant is tasked with figuring out the important nouns such as objects' name or activity or place according to the the hint, the question, and the image. /'''
                }
            ]
        },
        {
            ...
            "content": [
                {
                    ...
                    "text": f'''To answer the question "{question}", what things should be searched on wikipedia to get more background information? Select two to three things in the image and each one should be described in one or two words. The hint is that I have already know there are "{objs}" in the image. '''
                },
                {
                    'image': local_file_path
                },
...
# In order to answer the question: "{question}", what objects in the image do you think are important? List their name in the format of json.The image is as follows.
    print(messages)
    response = MultiModalConversation.call(model='qwen-vl-max', messages=messages,api_key=api_key)
    return response
# AI assistant is tasked with helping me figure out the important objects' name according to the the hint, the question, and the image. /
def create_payload_describe_objs(image: str, question: str , objs):
    """Creates the payload for the API request."""
            ...
            "content": [
                {
                    ...
                    "text": '''You are a smart AI assistant and you should describe the listed objects. /'''
                }
            ]
        },
        {
            ...
            "content": [
                {
                    ...
                    "text": f'''The listed objects are "{objs}". Describe these objects in a few sentences seperately.'''
                },
                {
                    'image': local_file_path
                },
